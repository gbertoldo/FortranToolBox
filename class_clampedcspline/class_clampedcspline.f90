...
!>
!! \brief Class class_clampedcspline construct a clamped cubic spline from a set of
!!        data (x,y) and derivatives dy/dx at boundaries. After constructed, y(x)
!!        may be calculated within [xa, xb], where xa and xb are the lower and upper
!!        limits of the data set used to construct the spline.
!!        For technical details, see:
!!          - Faires, J. D. & Burden, R. Thomson/Brooks/Cole (Ed.) Numerical methods.
!!
module mod_class_clampedcspline
   use mod_tdma
   ...
   !> \brief Class class_clampedcspline for clamped cubic spline construction
   !!
   !!        y(x) = yi + b (x-xi) + c (x-xi)^2 + d (x-xi)^3, for xi <= x <= xi+1
   !!
   type, public :: class_clampedcspline
...
contains
   ...
   subroutine init(this, x, y, dydx_l, dydx_u)
      implicit none
      class(class_clampedcspline)       :: this   !< A reference to this object
      real(8),               intent(in) :: x(:)   !< Set of independent coordinates
      real(8),               intent(in) :: y(:)   !< Set of dependent coordinates
      real(8),               intent(in) :: dydx_l !< dy/dx on the lower boundary
      real(8),               intent(in) :: dydx_u !< dy/dx on the upper boundary
      ...
      call get_ccsplines_coeff(this%n-1, dydx_l, dydx_u, x, y, this%b, this%c, this%d) ! Output: last three
