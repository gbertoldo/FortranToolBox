...
program main
   ...
   use mod_class_clampedcspline

   implicit none

   ! Creating an instance of the clampedcspline class
   type(class_clampedcspline) :: spline

   ! Defining a set of data
   integer              :: opt = 1 ! 1=without fictitious points, 2=with fictitious points
   integer              :: N   = 10
   real(8), allocatable :: x(:)
   real(8), allocatable :: y(:)
   real(8)              :: dydx_l = 0.d0 ! dy/dx on the lower boundary
   real(8)              :: dydx_u = 0.d0 ! dy/dx on the upper boundary

   ! Getting points for interpolation
   call get_dataset(opt, N, x, y)

   ! Initializing the spline
   call spline%init(x, y, dydx_l, dydx_u)
